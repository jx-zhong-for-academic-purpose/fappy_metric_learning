...
class LabelShufflingLayer(caffe.Layer):
    def setup(self, bottom, top):
        ...
        self.mean_file_ = "no mean_file"
        self.mean_ = None

        if 'subtract' in layer_params :
            self.subtract_ = layer_params['subtract']

        if 'mean_file' in layer_params:
            self.subtract_ = np.zeros_like(self.scales_)
            self.mean_file_ = layer_params['mean_file']
            blob = caffe.proto.caffe_pb2.BlobProto()
            data = open(self.mean_file_, 'rb').read()
            blob.ParseFromString(data)
            self.mean_ = np.array(caffe.io.blobproto_to_array(blob))
    ...
    def forward(self, bottom, top):
        ...
        for i in xrange(len(self.shapes_)):
            if self.mean_ is None:
                ...
                top[0].data[...] = (np.array(batch_data[0]) - self.subtract_[0] - self.mean_) * self.scales_[0]
            else:
                top[1].data[...] = (np.array(batch_data[1]) - self.subtract_[1]) * self.scales_[1]
